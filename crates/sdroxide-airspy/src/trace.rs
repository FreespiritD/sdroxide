//! A self-contained session trace, so a user with an R2 can produce a report
...
//! block.
//!
//! # The two things it exists to settle
//!
//! **The rate doubling.** [`Trace::rate_plan`] records the complex rate asked
//! for, the programmed rate that follows from it, how that was encoded, and
//! what the receiver's own list said. A span that is half or double what it
//! should be is the failure mode here, and it looks like a working receiver.
//!
...
/// A shared, bounded record of one Airspy R2 session.
///
...
impl Trace {
    ...
    pub fn ctrl(
        ...
        tracing::debug!(target: "airspy::ctrl", request, name, value, index, want, ?got, outcome);
    }
    ...
    pub fn dump(&self) -> String {
        ...
        out.push_str("=== sdroxide Airspy R2/Mini session trace ===\n");
...
mod tests {
    ...
    fn dump_reports_requests_and_the_identity() {
        let t = Trace::new();
        t.set_identity("Airspy R2, firmware AirSpy NOS v1.0.0-rc10");
        t.ctrl(25, "GetSamplerates", 0, 0, 8, Some(8), "20.000M 5.000M");
        let d = t.dump();
        assert!(d.contains("req 25 GetSamplerates"), "{d}");
        assert!(d.contains("rc10"), "{d}");
    }
    ...
    fn the_rate_plan_shows_both_rates_and_the_encoding() {
        let t = Trace::new();
        t.rate_plan(10.0e6, 20.0e6, "Khz(20000)", &[20.0e6, 5.0e6]);
        let d = t.dump();
        assert!(d.contains("10.000 Msps complex"), "{d}");
        assert!(d.contains("20.000 Msps programmed"), "{d}");
        assert!(d.contains("Khz(20000)"), "{d}");
        assert!(d.contains("20.000M, 5.000M"), "{d}");
    }
